...
public class WordsGraph {
    ...
    public WordsGraph(ArrayList<MySentence> sentences) {
        nodes = new ArrayList<>();
    ...
    void findPath(ArrayList<MyToken> s, GraphNode node, int pri) {
...
//            newPath.getScore();
            ...
            for (GraphNode next : node.nextNodes) {
                for (int i = 0; i < next.list.size(); i++) {
                    ...
                    if (nextToken.iSentence == token.iSentence && nextToken.iPosition > token.iPosition
                            && nextToken.iPosition < token.iPosition + M) {
    ...
    public ArrayList<MySentence> combination() {
        graphing();
        for (MySentence sentence : candidates) {
